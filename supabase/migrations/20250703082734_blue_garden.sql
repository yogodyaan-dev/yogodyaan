/*
  ...
  1. New Tables
    - Enhanced booking system with scheduled classes
    - Waitlist management
...
DO $$ 
BEGIN
    -- Drop existing policies on class_bookings if they exist
...
EXCEPTION
    WHEN undefined_table THEN
        NULL; -- Table doesn't exist yet, which is fine
END $$;
...
DO $$
BEGIN
    -- Add foreign key for scheduled_classes to class_types
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'scheduled_classes_class_type_id_fkey'
    ) THEN
        ALTER TABLE scheduled_classes 
        ADD CONSTRAINT scheduled_classes_class_type_id_fkey 
        FOREIGN KEY (class_type_id) REFERENCES class_types(id);
    END IF;

    -- Add foreign key for scheduled_classes to instructors
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'scheduled_classes_instructor_id_fkey'
    ) THEN
        ALTER TABLE scheduled_classes 
        ADD CONSTRAINT scheduled_classes_instructor_id_fkey 
        FOREIGN KEY (instructor_id) REFERENCES instructors(id);
    END IF;

    -- Add foreign key for class_bookings to scheduled_classes
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'class_bookings_scheduled_class_id_fkey'
    ) THEN
        ALTER TABLE class_bookings 
        ADD CONSTRAINT class_bookings_scheduled_class_id_fkey 
...
-- Add foreign key for instructor_availability
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
...
-- Triggers for participant count (drop first if they exist)
