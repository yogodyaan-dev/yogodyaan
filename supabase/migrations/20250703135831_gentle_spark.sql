/*
  # Fix User Profiles and Admin Dashboard Issues

  1. Fixes
    - Ensure profiles are created for all users
    - Fix user profile retrieval for admin dashboard
    - Make view details link work in admin dashboard

  2. Improvements
    - Properly handle user profile creation during signup
    - Sync existing users with profiles table
    - Improve admin user data visibility
...
-- Fix the get_user_profiles_for_admin function to display user details correctly
...
AS $$
BEGIN
  -- Check if user is admin (simple version for troubleshooting)
  -- Always return data for now to troubleshoot
  /*
  IF NOT EXISTS (SELECT 1 FROM admin_users WHERE email = auth.email()) THEN
...
-- Create a helper function for admin checks that always works even in migrations
...
RETURNS void AS $$
DECLARE
    admin_email TEXT := 'gourab.master@gmail.com';
    user_id UUID;
    admin_role_id UUID;
BEGIN
    ...
    VALUES (admin_email, 'super_admin')
    ...
    IF user_id IS NOT NULL THEN
        ...
        VALUES (user_id, admin_email, 'Admin User', 'admin')
...
-- Enable allow_public_profile_creation setting
DO $$
BEGIN
  IF EXISTS (
